...
public class RectangleP
{
    ...
    {
       // Location = new Point(Location.X, Location.Y);
        PointF local = new PointF(Location.X, Location.Y);
        Matrix mx = new Matrix();
        mx.RotateAt(-_Angle, _Rectangle.Location);
        PointF[] Points = new PointF[] { local };
        mx.TransformPoints(Points);
       // Location = Points[0];
        RectangleF rect = new RectangleF(_Rectangle.X,_Rectangle.Y, Width, Height);
    ...
    private void SetPoints(Point P1, Point P2, Point P3, Point P4)
    {
        ...
        double width = Math.Sqrt(Math.Pow(P2.X - P1.X, 2) + Math.Pow(P2.Y - P1.Y, 2));
        double height = Math.Sqrt(Math.Pow(P3.X - P2.X, 2) + Math.Pow(P3.Y - P2.Y, 2));
